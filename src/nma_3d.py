...
class NMA_3D:
    ...
    def __init__(
        ...
        assert isinstance(ELN_params['vz_range'], (list, tuple, np.ndarray)), 'Type of ELN_params["vz_range"] must be from (list, tuple, np.ndarray)'
        assert 'phi_range' in ELN_params.keys(), "Require key 'phi_range' in phasespace_range."
        assert isinstance(ELN_params['phi_range'], (list, tuple, np.ndarray)), 'type(ELN_params["phi_range"]) is not in (list, tuple, np.ndarray)'
        ...
        assert isinstance(ELN_params['nvz'], int), 'ELN_params[nvz] must be an integer'
        assert isinstance(ELN_params['nphi'], int), 'ELN_params[nph] must be an integer'
        ...
        self.dvz, self.dphi = (self.vz_max-self.vz_min)/self.nvz, (self.phi_max-self.phi_min)/self.nphi
    ...
    def eps_0(self): # [Tested; stat:Success]
        e_0 = lambda v, phi : 1*self.ELN(v, phi)
        return sint.dblquad(
            e_0,
    ...
    def coeff_matrices_sym_pres(self):
        ...
        Az = np.diag(
                np.array(
                    [self.vz[int(i/self.nphi)] for i in range(self.nphi*self.nvz)]
                )
            )
        ...
        I0 = np.outer(
                ...
                np.array(
                    [self.ELN(self.vz[int(i/self.nphi)], self.phi[int(i%self.nphi)]) for i in range(self.nphi*self.nvz)]
                ).reshape(1, -1)
            )

        Ix = np.outer(
                np.array(
                    [np.sqrt( 1 - self.vz[int(i/self.nphi)]**2 )*np.cos(self.phi[int(i%self.nphi)] ) for i in range(self.nphi*self.nvz)]
                ).reshape(-1, 1),
                np.array(
                    [np.sqrt( 1 - self.vz[int(i/self.nphi)]**2)*np.cos(self.phi[int(i%self.nphi)]) * self.ELN(self.vz[int(i/self.nphi)], self.phi[int(i%self.nphi)]) for i in range(self.nvz*self.nphi)]
                ).reshape(1, -1)
            )

        Iy = np.outer(
                np.array(
                    [np.sqrt( 1 - self.vz[int(i/self.nphi)]**2 )*np.sin(self.phi[int(i%self.nphi)] ) for i in range(self.nphi*self.nvz)]
                ).reshape(-1, 1),
                np.array(
                    [np.sqrt( 1 - self.vz[int(i/self.nphi)]**2)*np.sin(self.phi[int(i%self.nphi)]) * self.ELN(self.vz[int(i/self.nphi)], self.phi[int(i%self.nphi)]) for i in range(self.nvz*self.nphi)]
                ).reshape(1, -1)
            )

        Iz = np.outer(
                np.array(
                    [self.vz[int(i/self.nphi)] for i in range(self.nvz*self.nphi)]
                ).reshape(-1, 1),
                np.array(
                    [self.vz[int(i/self.nphi)] *self.ELN(self.vz[int(i/self.nphi)], self.phi[int(i%self.nphi)]) for i in range(self.nvz*self.nphi)]
                ).reshape(1, -1)
            )
            ...
            for ikz, kz in enumerate(kzs): 
                for iky, ky in enumerate(kys): 
                    for ikx, kx in enumerate(kxs): 
                        ...
                        if ikx % 10 == 0:
                            print(f"\r{progress[ikz%len(progress)]} {int(ikz*100/len(kzs))}% [{ikz=} {iky=} {ikx=}]", end="")
                            sys.stdout.flush()

                        M = self.characteristic_matrix(charMs, epsilons, kx=kx, ky=ky, kz=kz)
                        eival = la.eig(M)[0]
                        evi = 0.0
                        evr = 0.0
                        evi = np.abs(eival.imag).max()
                        idx = np.argmax(np.abs(eival.imag)) # Index of the max growth rate
                        evr = eival.real.max()
    ...
    def run(self, kxs, kys, kzs, store_to, mode='sym')->str:
